...
> [!IMPORTANT]
> When using C3 to create this project, select "no" when it asks if you want to deploy. You need to follow this project's [setup steps](https://github.com/cloudflare/templates/tree/main/restless-thunder-d230#setup-steps) before deploying.
...
```bash
npm create cloudflare@latest -- --template=cloudflare/templates/restless-thunder-d230
```

A live public deployment of this template is available at [https://restless-thunder-d230.templates.workers.dev](https://restless-thunder-d230.templates.workers.dev)
...
1. Install the project dependencies with a package manager of your choice:
   ...
   npm install
   ```
2. Create a [D1 database](https://developers.cloudflare.com/d1/get-started/) with the name "restless-thunder-d230-d1":
   ```bash
   npx wrangler d1 create restless-thunder-d230-d1
   ```
   ...and update the `database_id` field in `wrangler.json` with the new database ID.
3. Run the following db migration to initialize the database (notice the `migrations` directory in this project):
   ```bash
   npx wrangler d1 migrations apply --remote restless-thunder-d230-d1
   ```
